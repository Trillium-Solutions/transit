...
|  Field Name | Required | Details |
|  ------ | ------ | ------ |
|  trip_id | **Required** | The **trip_id** contains an ID that identifies a trip on which the specified frequency of service applies. Trip IDs are referenced from the [trips.txt](#tripstxt) file. If a block\_id is specified for the trip, this indicates the following, based other trip and frequency parameters: (A) if the trip travel time from first to last stop is less than the time specified in **headway_secs**, then this block\_id is assumed to apply for all vehicle trips implied by this frequencies.txt row. (B) if the trip travel time is greater than the time specified in **headway_secs** and the trip is a loop (identical first and last stops), this implies more than one vehicle block. In this case, the number of vehicle blocks is determined by dividing the headway by trip travel time and rounding up to the nearest whole number. (C) if the trip travel time is greater than the time specified in **headway_secs** and the trip is not a loop (different first and last stops), then the block\_id is ignored. |
